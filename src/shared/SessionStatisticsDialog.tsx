...
type SessionStatisticsDialogProps = {
  ref: Ref<HTMLDialogElement>
  playLabel: 'Bets' | 'Rounds'
...
export function SessionStatisticsDialog({ ref, playLabel, statistics }: SessionStatisticsDialogProps) {
  ...
  const hasPushes = statistics.pushes !== undefined
      ...
      <form method="dialog" className="p-4 sm:p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#b1bad3]">Settled {playLabel.toLowerCase()} from this session</p>
          </div>
          <button
            type="submit"
        ...
        <div className={`mt-5 grid grid-cols-2 gap-2 ${hasPushes ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}`} aria-live="polite">
          ...
          <Stat label={playLabel} value={String(statistics.bets)} />
          ...
          {statistics.pushes !== undefined ? <Stat label="Pushes" value={String(statistics.pushes)} /> : null}
        ...
        <section className="mt-5 rounded bg-[#0f212e] p-4" aria-labelledby="profit-history-title">
          ...
          {statistics.bets === 0 ? (
            <p className="mt-2 text-center text-xs text-[#b1bad3]">Your settled {playLabel.toLowerCase()} will appear here.</p>
          ) : null}
        </section>
      </form>
    </dialog>
