export const ROW_OPTIONS = [8, 9, 10, 11, 12, 13, 14, 15, 16] as const

export type RowCount = (typeof ROW_OPTIONS)[number]
export type Risk = 'low' | 'medium' | 'high'

export const binPayouts: Record<RowCount, Record<Risk, readonly number[]>> = {
  8: {
...
export function getBinColor(index: number, count: number) {
  const center = (count - 1) / 2
  const edgeDistance = Math.abs(index - center) / center
  return `rgb(255 ${Math.round(191 * (1 - edgeDistance))} ${Math.round(63 * edgeDistance)})`
}
